                ...
                <div class="table-responsive  export-table">
                    <table id="pending-schools-datatable"
                        ...
                        <?php if (!empty($smstypes)) : ?>
                                <?php foreach ($smstypes as $key => $smstype) : ?>
                                    <tr>
                                        <td><?= $key + 1; ?></td>
                                        <td><?= esc($smstype->name); ?></td>
                                        <td><div data-id="<?= esc($smstype->id); ?>" class="main-toggle main-toggle-secondary <?= $smstype->status == '0'? 'on':'off' ; ?> sms-type-switch-btn">
											<span></span>
										</div> </td>
                                        <td><?= esc($smstype->description); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
